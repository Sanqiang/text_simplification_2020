...
#SBATCH --gres=gpu:1
#SBATCH --job-name=train_t2t_controlsplit_encf_val
#SBATCH --output=train_t2t_controlsplit_encf_val.out
#SBATCH --nodes=1
...
srun python ../../ts_model/run.py \
    ...
    --lr 0.1 \
    --num_ref 8 \
    --control_mode "length:syntax:split:ppdb:predict:encoder:val:fix_predict" \
    --max_ppdb_len 100 \
